...
struct LibraryDetailPane: View {
    ...
    let onLaunchDiagnostics: (Bottle) -> Void
...
/// The actual pane content for one bottle. `@ObservedObject`, not `let` —
/// without it, `updateInstalledPrograms()` completing asynchronously (the
/// scan that makes `canLaunch` true for a bottle nothing has scanned yet)
/// never triggers a re-render, and Play stays visibly disabled even after the
/// scan succeeds. This is the same mistake `GameTile` and `LibraryRow` avoid
/// by declaring `bottle` as `@ObservedObject` themselves.
private struct LibraryDetailPaneContent: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                EntryDetailAdvancedSection(bottle: bottle,
                                           onRunProgram: { actions.runProgram(bottle, $0) },
                                           onLaunchDiagnostics: { onLaunchDiagnostics(bottle) })
            }
            .padding(24)
        }
        .id(bottle.id)
        ...
        .onChange(of: renameRequestID) { _, id in
            guard id == bottle.id else { return }
            isRenaming = true
            renameRequestID = nil
        }
    }
    ...
    private var playButton: some View {
        ...
        .popover(isPresented: $showProgramPicker, arrowEdge: .bottom) {
            ProgramPickerList(programs: bottle.userVisiblePrograms) { program in
                ...
                actions.runProgram(bottle, program)
            }
        }
...
/// The dominant Play control's fill/press/glow. Kept local — this is the one
/// place the launcher's "everything else recedes" accent treatment applies,
/// and `PrimaryCTAButtonStyle` is tuned for `CrosswireTheme`, not the dark
/// launcher tokens (white text is right in both, but the glow is new here).
private struct LauncherPlayButtonStyle: ButtonStyle {
